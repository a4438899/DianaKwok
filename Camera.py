...
cap = cv2.VideoCapture('market4.m4v')
...
def add_to_t_tracks(h_tracks, t_tracks, min_center):
    ...
    if (len(h_tracks) == 1 and len(opn_tracks) == 1 and len(t_tracks) == 1):
        ...
        if (min_value_t > 40):  # 新目标
            print('new object')
            for i in range(len(t_tracks)):
                if t_tracks[i] == [] and is_inserted[i] != 1:
                    t_tracks[i].append(min_center)
                    is_inserted[i] = 1
                    break
                else:
                    continue
...
'''
    ...
    if (h_tracks != [] and t_tracks == []):
        ...
        if (len(h_tracks) < len(opn_tracks)):
            ...
            for i in is_inserted:
                if (i == 0):  # 说明没添加过中点
                    ...
                    if (len(t_tracks) < 1):
                        break
                    # if(x2 <= 20 or x2 >= width - 20 or y2 <= 20 or y2 >= height - 20):
                    print('out')
                    # 目标出去
                    del (t_tracks[x])
                    i = 1
        ...
        # 有新目标或上一帧掉帧
        if (len(h_tracks) > len(opn_tracks)):
            diff = len(h_tracks) - len(t_tracks)
            if (diff != 0):
                for count_new in range(diff):
                    new_list = []  # 几个目标就创几个list
                    t_tracks.append(new_list)
            is_inserted = is_inserted = np.zeros((len(t_tracks),), dtype=np.int)
            for i in range(len(h_tracks)):
    ...
    cv2.putText(frame1, "count: {}".format(f'{len(t_tracks)}'), (10, 20), cv2.FONT_HERSHEY_SIMPLEX,
                1, (0, 0, 255), 3)
